  ...
  try {
    ...
    fastify.setErrorHandler((error, request, reply) => {
      ...
      if (err && err.statusCode === 429) {
        return reply
          ...
          .send({
            statusCode: 429,
            error: err.error || "Too Many Requests",
            message: err.message,
          });
      }
      ...
      } else if (error instanceof InvalidImageOperationError) {
    ...
    fastify.get(
      ...
      async (req: FastifyRequest<{ Params: { "*": string } }>, rep) => {
        ...
        if (lnkIndex === -1)
          return rep.code(400).send("Malformed internal route");
    ...
    fastify.post("/warmup", async (req, rep) => {
      ...
      return rep.send({
        success: true,
        message: `${targets.length} variantes mises en cache.`,
      });
    });
    ...
    fastify.delete("/cache", async (req, rep) => {
      ...
      if (!authHeader || authHeader !== secretToken) {
        return rep.status(401).send({
          ...
          error: "Unauthorized",
          message:
            "Jeton de purge invalide ou manquant dans les en-têtes (X-Purge-Token).",
        });
      }
